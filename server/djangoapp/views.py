...
# user log out function **changes needs to be done
...
def add_review(request):
    if request.user.is_authenticated:  # Check if the user is logged in
        data = json.loads(request.body)  # Parse the review data from the request body
        try:
            response = post_review(data)  # Call the post_review method to submit the review
            if response:
        ...
        return JsonResponse({"status": 403, "message": "Unauthorized"})  # If not logged in, return Unauthorized
